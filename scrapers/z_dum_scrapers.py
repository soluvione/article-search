    ...
    try:
        ...
        send_notification(
            GeneralError(f"Error encountered while checking issue scan status for TK journal "
                         f"with path = {kwargs['logs_path']}, (check_scan_status, z_dum_scrapers.py). Error: {e}"))
        raise e


def z_dum_scraper(journal_name, start_page_url, pdf_scrape_type, pages_to_send, parent_type, file_reference):
    i = 0
    try:
        # Webdriver options
                    ...
                    try:
                        ...
                        send_notification(
                            GeneralError(f"No pdf link found for TK article of journal {journal_name} "
                                         f"(z_dum_no_ref_scraper, z_dum_scrapers). "
...
if __name__ == '__main__':
    z_dum_scraper('foo', 'bar', 'zoo', 'poo', 'A_KLNK', 1,)
