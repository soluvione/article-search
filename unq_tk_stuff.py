...
with webdriver.Chrome(service=service, options=options) as driver:
    ...
    # author_affiliations = main_element.find_elements(By.CSS_SELECTOR, 'div[class="article-author"]')[1].text
    data = "aDivision of Medical Oncology, Marmara University Faculty of Medicine, İstanbul, Türkiye bDepartment of Internal Medicine, Marmara University Faculty of Medicine, İstanbul, Türkiye cClinic of Medical Oncology, Erzurum Training and Research Hospital, Erzurum, Türkiye dDepartment of Pathology, Marmara University Faculty of Medicine, İstanbul, Türkiye"
    print([data.index(found) for found in re.findall('[a-z][A-Z]', data)])
    array = [data.index(found) for found in re.findall('[a-z][A-Z]', data)]
    for i in range(len(array)):
        try:
            print(data[array[i]: array[i+1]][1:])
            i += 1
        except:
